...
class AppCalculadoraLimites(ctk.CTk):
    def __init__(self):
        ...
        self.entrada_funcion = ctk.CTkEntry(self.panel_izquierdo, placeholder_text="Ej: (x**2 - 4) / (x - 2)", width=340)
        self.entrada_funcion.pack(pady=5)

        self.lbl_h = ctk.CTkLabel(self.panel_izquierdo, text="Punto al que tiende x (Valor h):", font=("Arial", 14))
        self.lbl_h.pack(anchor="w", padx=30, pady=5)
        
        self.entrada_h = ctk.CTkEntry(self.panel_izquierdo, placeholder_text="Ej: 2", width=340)
        ...
        self.mostrar_texto("Esto es una calculadora y graficadora de limites")

        # Permite crear grafico
    ...
    # Acción de calcular y graficar
    def evento_calcular(self):
        funcion_texto = self.entrada_funcion.get()
        valor_h_texto = self.entrada_h.get()

        if not funcion_texto or not valor_h_texto:
            self.mostrar_texto("Error: Por favor rellene ambos campos.")
            return

        try:
           
            # Funciones benja varela
            
        
            # Ejemplo de salida en texto:
            texto_resultado = f"Función: {funcion_texto}\nTendencia x -> {valor_h_texto}\n\n[Resultados aquí]"
            self.mostrar_texto(texto_resultado)

            # Ejemplo para refrescar gráfico:
            # self.ax.clear()
            # self.ax.plot(lista_x, lista_y)
            # self.canvas.draw()

        except Exception as e:
            self.mostrar_texto(f"Error en el proceso:\n\n{str(e)}")
